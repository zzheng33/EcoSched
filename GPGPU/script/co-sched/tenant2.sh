#!/bin/bash
# Tenant 2: NUMA 1 (CPUs 48-95,144-191)
# Usage: bash run2.sh --app gups --gpus 3
#        bash run2.sh --app resnet50 --gpus 2,3 [--batch-size 2048] [--epochs 100] [--lr 0.001]
#        bash run2.sh --app bert --gpus 2,3

set -euo pipefail

CPUBIND="48-95,144-191"
MEMBIND=1
...
if [[ -z "$APP" || -z "$GPUS" ]]; then
    echo "Usage: bash run2.sh --app <name> --gpus <id,id,...> [--batch-size N] [--epochs N] [--lr F]"
    exit 1
fi
...
echo "[INFO] Tenant 2 | App: $APP | GPUs: $GPUS | NUMA: $MEMBIND | CPUs: $CPUBIND"
