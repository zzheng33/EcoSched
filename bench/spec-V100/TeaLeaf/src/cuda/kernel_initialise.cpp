...
void run_kernel_initialise(Chunk *chunk, Settings &settings, int comms_lr_len, int comms_tb_len) {
  ...
  // Automatic GPU distribution based on MPI rank
  if (!settings.device_selector) {
    selected = settings.rank % count;
    print_and_log(settings, "# Automatically selecting device %d for rank %d (rank %% %d devices)\n", selected, settings.rank, count);
  } else {
    try {
